...
void HotfixDatabaseConnection::DoPrepareStatements()
{
    ...
    //JournalInstance.db2
    PrepareStatement(HOTFIX_SEL_JOURNAL_INSTANCE, "SELECT Name, Description, ID, MapID, BackgroundFileDataID, ButtonFileDataID, ButtonSmallFileDataID, LoreFileDataID, OrderIndex, Flags, AreaID FROM journal_instance ORDER BY ID DESC", CONNECTION_SYNCH);
    ...
    PrepareStatement(HOTFIX_SEL_MAP_CHALLENGE_MODE, "SELECT Name, ID, MapID, Flags, CriteriaCount1, CriteriaCount2, CriteriaCount3 FROM map_challenge_mode ORDER BY ID DESC", CONNECTION_SYNCH);
