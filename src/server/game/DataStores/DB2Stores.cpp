...
DB2Storage<MapChallengeModeEntry>               sMapChallengeModeStore("MapChallengeMode.db2", MapChallengeModeInfo::Instance());
...
typedef std::unordered_map<uint32, std::vector<const JournalEncounterItemEntry*>> JournalEncounterItemContainer;
typedef std::unordered_map<uint32, std::vector<const JournalEncounterEntry*>> JournalEncounterContainer;
typedef std::unordered_map<uint32/*mapID*/, const JournalInstanceEntry*> JournalInstanceContainer;
...
namespace
{
    ...
    JournalEncounterItemContainer _journalEncounterItem;
    JournalEncounterContainer _journalEncounter;
    JournalInstanceContainer _journalInstance;
...
#undef LOAD_DB2
    ...
    //for(const JournalInstanceEntry* journalInstance : sJournalInstanceStore)
    //
...
const std::vector<const JournalEncounterItemEntry*>* DB2Manager::GetJournalItem(uint32 encounterId) const
{
    auto itr = _journalEncounterItem.find(encounterId);
    if (itr != _journalEncounterItem.end())
        return &itr->second;
    return nullptr;
}
const std::vector<const JournalEncounterEntry*>* DB2Manager::GetJournalEncounter(uint32 instanceId) const
{
    auto itr = _journalEncounter.find(instanceId);
    if (itr != _journalEncounter.end())
        return &itr->second;
    return nullptr;
}
const JournalInstanceEntry* DB2Manager::GetJournalInstanceMap(uint32 mapId) 
{
    auto itr = _journalInstance.find(mapId);
    if (itr != _journalInstance.end())
        return itr->second;
    return nullptr;
}
